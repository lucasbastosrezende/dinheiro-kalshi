...
class AutoTrader {
  ...
  sizeOrder(opportunity) {
    ...
    const contratos = Math.max(0, Math.min(Math.floor(valor / preco), a.maxContractsPerOrder));
    return { contracts: contratos, price: preco, estimatedCost: +(contratos * preco).toFixed(2) };
  }
  ...
  async runOnce(analysis) {
    ...
    if (this.state.mode === 'semi') {
      ...
      for (const o of analysis.bestOverall.slice(0, 8)) {
        ...
        if (tamanho.contracts < 1) continue;
    ...
    if (tamanho.contracts < 1) {
      const rec = this._log({ type: 'skip', reason: 'o valor daria menos de um contrato', ticker: escolhida.ticker });
      ...
      const r = await this._executar(
        {
          ...
          estimatedCost: +(s.contracts * preco).toFixed(2),
  ...
  _conferirOrdem(ordem) {
    ...
    if (!Number.isInteger(ordem.contracts) || ordem.contracts < 1) erros.push('quantidade inválida');
    ...
    const custo = ordem.contracts * ordem.price;
  ...
  async _executar(ordem, analysis) {
    ...
    try {
      ...
      const gasto = executados * medioNoLado;
