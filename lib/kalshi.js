...
class KalshiClient {
  ...
  getMarket(ticker) { return this.publicGet(`/markets/${encodeURIComponent(ticker)}`); }
  getBRTI() { return this.signedRequest('GET', '/cfbenchmarks/latest_values?id=BRTI'); }

  async portfolioPages(subPath, field) {
    const out = []; let cursor = ''; const seen = new Set();
    do {
      const page = await this.signedRequest('GET', subPath + (cursor ? '&cursor='+encodeURIComponent(cursor) : ''));
      if (!Array.isArray(page[field])) throw new Error('resposta de portfólio incompleta');
      out.push(...page[field]); cursor = page.cursor || '';
      if (cursor && seen.has(cursor)) throw new Error('cursor repetido no portfólio');
      seen.add(cursor);
    } while (cursor);
    return { [field]: out };
  }
  ...
  getPositions() {
    return this.portfolioPages('/portfolio/positions?limit=200', 'market_positions');
  }

  getOrders(status) {
    return this.portfolioPages(`/portfolio/orders?limit=200${status ? `&status=${status}` : ''}`, 'orders');
  }
  ...
  createOrder({ ticker, side, priceDollars, count, clientOrderId, timeInForce = 'immediate_or_cancel' }) {
    ...
    // Cotação YES com quatro casas, sem piorar o limite aprovado por arredondamento.
    const yesPrice = side === 'yes' ? preco : 1 - preco;
    if(Math.abs(yesPrice*10000-Math.round(yesPrice*10000))>1e-7)throw new Error('preço com precisão não suportada');
...
module.exports = { KalshiClient, HOSTS };
