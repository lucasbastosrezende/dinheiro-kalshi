...
// Descobre se a serie pergunta pelo preco FINAL ou por ENCOSTAR num valor.
// Erra para o lado seguro: na duvida trata como preco final, que e o formato mais comum.
function detectContractModel(seriesTicker, title) {
  const s = String(seriesTicker || '').toUpperCase();
  const t = String(title || '').toLowerCase();
  if (/MAX|MIN|ATH|PRICE/.test(s)) return 'touch';
  if (/how high|how low|one touch|will .* hit |hit \$|cross|reach/.test(t)) return 'touch';
  return 'terminal';
}
...
// Taxa de negociacao Kalshi: ceil(rate * C * P * (1-P)) em centavos.
function tradingFeeDollars(priceDollars, contracts, rate) {
  const cents = Math.ceil(rate * contracts * priceDollars * (1 - priceDollars) * 100);
  return cents / 100;
}
...
function classifyStrike(rawType, floorStrike, capStrike) {
  ...
  if (temPiso && temTeto) return 'range';
  if (temTeto) return 'below';
  if (temPiso) return 'above';
  return 'unknown';
}
...
function evaluateLeg({ side, market, modelProbYes, fees, yearsToClose, sizeContracts = 1 }) {
  ...
  const tradable = price > 0 && price < 1;
  const feeIn = tradable ? tradingFeeDollars(price, sizeContracts, fees.tradingFeeRate) : 0;
  const cost = price * sizeContracts + feeIn;
  const payoff = 1 * sizeContracts;
  ...
  const grossReturnPct = price > 0 ? (1 - price) / price : 0; // retorno se ganhar
  const lossPct = -1;                                         // perde 100% do custo se errar

  // Kelly para aposta binaria: f* = (p*b - q)/b, b = odds liquidas
  const b = price > 0 ? (1 - price) / price : 0;
  const kelly = b > 0 ? clamp((modelProb * b - (1 - modelProb)) / b, 0, 1) : 0;

  // Custo de saida imediata (medida de liquidez real)
  const roundTripCost = tradable && exitBid > 0 ? price - exitBid : price;
  ...
  return {
    ...
    lossPct,
    kelly,
    feePerContract: feeIn,
    breakevenProb: price + tradingFeeDollars(price, 1, fees.tradingFeeRate),
    roundTripCost,
    maxLossPerContract: cost,
    maxGainPerContract: payoff - cost,
  };
}
...
function analyze({ rawMarkets, spot, candles, config, now = Date.now() }) {
  ...
  const contractModel = detectContractModel(
    config.seriesTicker || String(config.eventTicker || '').split('-')[0],
    active.length ? active[0].title : ''
  );

  const volRealized = realizedVolAnnual(candles, config.model.ewmaHalfLifeMinutes);
  const volImplied = impliedVolFit(active, spot, contractModel, now, yearsToClose);
  ...
  else sigma = volRealized || volImplied || 0.5;
  ...
  for (const m of active) {
    ...
    const msFaixa = m.closeTime ? Math.max(0, new Date(m.closeTime).getTime() - now) : msToClose;
    const anosFaixa = msFaixa / MS_PER_YEAR;

    const probYes = modelProbForMarket(m, spot, sigma, anosFaixa, contractModel);
    const modelConfiavel = probYes != null;
    ...
    const modelProbYes = modelConfiavel ? probYes : m.mid;

    const refStrike = m.kind === 'below' ? m.capStrike : m.floorStrike || m.strike;
    const sigmaMoves = sigmaPeriod > 0 && refStrike > 0 ? Math.log(spot / refStrike) / sigmaPeriod : 0;
    const distancePct = m.strike > 0 ? (spot - m.strike) / spot : 0;

    const legs = ['yes', 'no'].map((side) => {
      const leg = evaluateLeg({ side, market: m, modelProbYes, fees: config.fees, yearsToClose: anosFaixa });
      const parts = {
      ...
      if (!modelConfiavel) filtersFailed.push('este tipo de aposta nao tem modelo de probabilidade');
