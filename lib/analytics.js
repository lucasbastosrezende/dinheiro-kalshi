...
function evaluateLeg({ side, market, modelProbYes, fees = {}, yearsToClose, sizeContracts = 1, capitalAtual, capitalInicial }) {
  ...
  const feeOut = tradable && exitBid > 0 && exitBid < 1 ? tradingFeeDollars(exitBid, 1, fees.tradingFeeRate ?? 0.07) : 0;
...
function analyze({ rawMarkets, spot, candles, config, now = Date.now() }) {
  ...
  for (const m of active) {
    ...
    const sigmaMoves = sigmaPeriod > 0 && m.strike > 0 ? Math.log(spot / m.strike) / sigmaPeriod : 0;
